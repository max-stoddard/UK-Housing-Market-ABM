...
- The grouping below was cross-checked against the current `input-data-versions/v5o3/config.properties`, `input-data-versions/dashboard-input-version-history.json`, and `input-data-versions/CALIBRATION_PARAMETER_CHANGELOG.md`.
- `BTL_PROBABILITY_MULTIPLIER`, `PSYCHOLOGICAL_COST_OF_RENTING`, `SENSITIVITY_RENT_OR_PURCHASE`, `BTL_CHOICE_INTENSITY`, and `MARKET_AVERAGE_PRICE_DECAY` remain outside the remaining set after the `v5o3` strict TuRBO five-parameter promotion from `v4.26`. The promoted local-refinement candidate improves 10-seed 2024 aggregate validation loss and constrained HPI metric losses versus `v4.26`; the retained evidence notes that `v5o3` also improves aggregate loss versus `v5o2`, but worsens `core_hpiCyclePeriod` loss versus `v5o2`.
