...
- The grouping below was cross-checked against the current `input-data-versions/v5o2/config.properties`, `input-data-versions/dashboard-input-version-history.json`, and `input-data-versions/CALIBRATION_PARAMETER_CHANGELOG.md`.
- `BTL_PROBABILITY_MULTIPLIER`, `PSYCHOLOGICAL_COST_OF_RENTING`, `SENSITIVITY_RENT_OR_PURCHASE`, `BTL_CHOICE_INTENSITY`, and `MARKET_AVERAGE_PRICE_DECAY` remain outside the remaining set after the `v5o2` exact raw-member five-parameter ES-MDA manual promotion from `v4.26`. The promoted iteration `3`, member `7` improves 10-seed 2024 aggregate validation loss versus `v4.26`; the retained evidence notes that fresh promoted-snapshot `core_hpiStd` loss worsens.
